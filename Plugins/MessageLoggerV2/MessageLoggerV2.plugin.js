...
module.exports = class MessageLoggerV2 {
  ...
  start() {
    let onLoaded = () => {
      try {
        if (!this.UserStore) this.UserStore = Webpack.Stores?.UserStore || Webpack.getStore?.('UserStore') || Webpack.getByKeys('getCurrentUser', 'getUser');
...
*/
    ...
    this.messageStore = Webpack.Stores?.MessageStore || Webpack.getStore?.('MessageStore') || Webpack.getByKeys('getMessages', 'getMessage');

    this.ChannelStore = Webpack.Stores?.ChannelStore || Webpack.getStore?.('ChannelStore') || Webpack.getByKeys('getChannel', 'getDMFromUserId');
    this.SelectedChannelStore = Webpack.Stores?.SelectedChannelStore || Webpack.getStore?.('SelectedChannelStore') || Webpack.getByKeys('getChannelId', 'getLastSelectedChannelId');

    if (!this.messageStore || !this.ChannelStore || !this.SelectedChannelStore) {
      throw new Error('Required Discord stores could not be found');
    }
    ...
    this.muteModule = Webpack.Stores?.UserGuildSettingsStore || Webpack.getStore?.('UserGuildSettingsStore') || Webpack.getModule(m => m?.isChannelMuted && m?.isMuted) || { isMuted: () => false, isChannelMuted: () => false };
    ...
    // Discord no longer consistently exposes the private _channelMessages map.
    // Prefer it when available, but emulate the small subset used by this plugin
    // through the public MessageStore methods on newer Discord builds.
    const plugin = this;
    const internalChannelMessages = Webpack.getByKeys('_channelMessages')?._channelMessages;
    this.usesInternalChannelMessages = !!internalChannelMessages;
    this.channelMessages = internalChannelMessages || new Proxy(Object.create(null), {
      get: (_target, channelId) => {
        if (typeof channelId !== 'string') return undefined;
        const messages = this.messageStore.getMessages?.(channelId);
        if (!messages && !this.messageStore.getMessage) return undefined;
        return {
          get: messageId => this.messageStore.getMessage?.(channelId, messageId) || messages?.get?.(messageId),
          get ready() {
            try {
              if (typeof plugin.messageStore?.isReady === 'function') return plugin.messageStore.isReady(channelId);
            } catch { }
            return typeof messages?.ready === 'boolean' ? messages.ready : true;
          }
        };
      },
      ownKeys: () => [],
    ...
    this.menu.randomValidChannel = (() => {
      const guildChannelStore = Webpack.Stores?.GuildChannelStore || Webpack.getStore?.('GuildChannelStore') || Webpack.getByKeys('getChannels');
      const channels = guildChannelStore?.getChannels?.() || {};
  ...
  invalidateAllChannelCache() {
    if (this.usesInternalChannelMessages) {
      for (let channelId in this.channelMessages) this.invalidateChannelCache(channelId);
    }
    this.forceReloadMessages();
  }
  invalidateChannelCache(channelId) {
    if (this.usesInternalChannelMessages && this.channelMessages[channelId]) {
      this.channelMessages[channelId].ready = false;
      return;
    }
    // MessageStore's public API does not expose a writable ready flag.
    // Refresh the selected channel and force the patched message components to render again.
    if (channelId && this.selectedChannel?.id === channelId) this.cacheChannelMessages(channelId);
    this.forceReloadMessages();
  }
  ...
  */
  ...
  async patchMessages() {
    ...
    if (!document.querySelector('li[class*="messageListItem"]')) {
      await Promise.race([
        new Promise(res => {
          const sub = this.subscribeToQuerySelector(() => (this.unsubscribeObserver(sub), res()), 'li[class*="messageListItem"]');
        }),
  ...
  forceReloadMessages() {
    try {
      this.dispatcher?.dispatch({ type: 'MLV2_FORCE_UPDATE_MESSAGE' });
      this.dispatcher?.dispatch({ type: 'MLV2_FORCE_UPDATE_MESSAGE_CONTENT' });
    } catch { }

    const chat = document.querySelector('[class*="chatContent_"], [class*="chatContent-"]');
    if (!chat) return;
    let fiber = BdApi.ReactUtils?.getInternalInstance?.(chat) || this.getReactInstance(chat);
    while (fiber) {
